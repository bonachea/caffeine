...
#  if __GNUC__ >= 16
     ...
     call no("NOTIFY") ! missing F2023 feature
     call no("IMAGE_INDEX_TEAM_NUMBER") ! missing team_number keyword
     call no("NUM_IMAGES_TEAM") ! missing team keyword
     call no("CO_MIN") ! CO_MIN(character) runtime crash
     call no("CO_MAX") ! CO_MAX(character) runtime crash
#  endif
#elif _CRAYFTN
   if (.not. stand_alone) return
